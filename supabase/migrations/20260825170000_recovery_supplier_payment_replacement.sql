-- One controlled replacement after a verified supplier-payment correction.
...
LOCK TABLE public.supplier_payments, public.supplier_payment_corrections,
...
CREATE UNIQUE INDEX IF NOT EXISTS supplier_payment_corrections_replacement_fkey_uidx
  ON public.supplier_payment_corrections
    (org_id, entity_id, vendor_id, original_payment_id, id);

CREATE TABLE IF NOT EXISTS public.supplier_payment_replacements (
  ...
  CONSTRAINT supplier_payment_replacements_number_check CHECK (
    replacement_number = btrim(replacement_number)
    AND length(replacement_number) BETWEEN 1 AND 80
    AND replacement_number !~ '[[:cntrl:]]'
  ),
  CONSTRAINT supplier_payment_replacements_reference_check CHECK (
    reference = btrim(reference)
    AND length(reference) BETWEEN 1 AND 240
    AND reference !~ '[[:cntrl:]]'
  ),
  CONSTRAINT supplier_payment_replacements_currency_check
    CHECK (currency ~ '^[A-Z]{3}$'),
  CONSTRAINT supplier_payment_replacements_amount_check CHECK (
    amount::text NOT IN ('NaN', 'Infinity', '-Infinity')
    AND amount > 0 AND round(amount, 2) = amount
  ),
  CONSTRAINT supplier_payment_replacements_key_check CHECK (
    idempotency_key = btrim(idempotency_key)
    AND length(idempotency_key) BETWEEN 1 AND 160
    AND idempotency_key !~ '[[:cntrl:]]'
    AND payload_hash ~ '^[0-9a-f]{32}$'
  ),
  CONSTRAINT supplier_payment_replacements_correction_fkey
    FOREIGN KEY (
      org_id, entity_id, vendor_id, original_payment_id,
      original_correction_id
    ) REFERENCES public.supplier_payment_corrections(
      org_id, entity_id, vendor_id, original_payment_id, id
    ),
...
CREATE UNIQUE INDEX IF NOT EXISTS supplier_payment_replacements_org_entity_id_uidx
  ON public.supplier_payment_replacements (org_id, entity_id, id);

LOCK TABLE public.supplier_payment_replacements IN SHARE ROW EXCLUSIVE MODE;
...
ALTER TABLE public.accounting_events
  ADD CONSTRAINT accounting_events_source_type_check CHECK (
    source_type IN (
      ...
      'customer_receipt_replacement', 'supplier_payment_replacement'
    )
  );
ALTER TABLE public.accounting_events
...
ALTER TABLE public.accounting_events
  ADD CONSTRAINT accounting_events_source_shape_check CHECK (
    ...
    OR (source_type IN (
      ...
      'supplier_payment_replacement'
    ) AND source_id IS NOT NULL)
  );

CREATE OR REPLACE FUNCTION public.guard_supplier_payment_replacement_write()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF current_setting('tapaano.supplier_payment_replacement_write', true)
       IS DISTINCT FROM 'trusted' THEN
    RAISE EXCEPTION 'supplier payment replacement is immutable; use the trusted replacement workflow';
  END IF;
...
BEGIN
  IF TG_TABLE_NAME = 'accounting_events' THEN
    ...
    ELSIF OLD.source_type = 'supplier_payment_replacement'
       AND current_setting('tapaano.supplier_payment_replacement_write', true)
         IS DISTINCT FROM 'trusted' THEN
      RAISE EXCEPTION 'supplier payment replacement accounting event is immutable';
    END IF;
  ELSIF TG_TABLE_NAME = 'journal_entries' THEN
    IF TG_OP = 'INSERT' AND NEW.reversal_of_id IS NOT NULL THEN
      ...
      ELSIF v_source_type = 'supplier_payment_replacement' THEN
        RAISE EXCEPTION 'supplier payment replacement reversal is unavailable';
      END IF;
    ELSIF TG_OP IN ('UPDATE', 'DELETE') THEN
      ...
      ELSIF v_source_type = 'supplier_payment_replacement'
         AND current_setting('tapaano.supplier_payment_replacement_write', true)
           IS DISTINCT FROM 'trusted' THEN
        RAISE EXCEPTION 'supplier payment replacement journal is immutable';
      END IF;
    END IF;
  ELSIF TG_TABLE_NAME = 'journal_lines' THEN
    IF TG_OP = 'TRUNCATE' THEN
      IF (current_setting('tapaano.customer_receipt_replacement_write', true)
          ...
          AND EXISTS (SELECT 1 FROM public.accounting_events
            WHERE source_type = 'customer_receipt_replacement'))
         OR (current_setting('tapaano.supplier_payment_replacement_write', true)
            IS DISTINCT FROM 'trusted'
          AND EXISTS (SELECT 1 FROM public.accounting_events
            WHERE source_type = 'supplier_payment_replacement')) THEN
        RAISE EXCEPTION 'settlement replacement journal lines are immutable';
      END IF;
      RETURN NULL;
    END IF;
    ...
    ELSIF v_source_type = 'supplier_payment_replacement'
       AND current_setting('tapaano.supplier_payment_replacement_write', true)
         IS DISTINCT FROM 'trusted' THEN
      RAISE EXCEPTION 'supplier payment replacement journal lines are immutable';
    END IF;
  END IF;
  IF TG_OP = 'DELETE' THEN RETURN OLD; END IF;
  RETURN NEW;
...
CREATE OR REPLACE FUNCTION public.validate_supplier_payment_replacement_graph(
...
AS $$
DECLARE
  v_replacement public.supplier_payment_replacements%ROWTYPE;
  ...
  v_credit numeric;
BEGIN
  SELECT * INTO v_replacement FROM public.supplier_payment_replacements
  ...
  IF v_replacement.replacement_number IS NULL
    ...
    RAISE EXCEPTION 'supplier payment replacement header is not canonical';
  END IF;
  ...
  IF NOT FOUND OR v_correction.org_id IS DISTINCT FROM v_replacement.org_id
     ...
     OR v_correction.vendor_id IS DISTINCT FROM v_replacement.vendor_id
     OR v_correction.original_payment_id
       IS DISTINCT FROM v_replacement.original_payment_id
    ...
    RAISE EXCEPTION 'supplier payment replacement correction graph is invalid';
  END IF;
  PERFORM public.validate_supplier_payment_correction_graph(v_correction.id);
  SELECT * INTO v_payment FROM public.supplier_payments
  WHERE id = v_replacement.original_payment_id;
  IF NOT FOUND OR v_payment.org_id IS DISTINCT FROM v_replacement.org_id
     OR v_payment.entity_id IS DISTINCT FROM v_replacement.entity_id
     OR v_payment.vendor_id IS DISTINCT FROM v_replacement.vendor_id
     OR v_payment.bill_id IS DISTINCT FROM v_replacement.bill_id
     OR upper(v_payment.currency) IS DISTINCT FROM v_replacement.currency
     OR v_payment.amount IS DISTINCT FROM v_replacement.amount THEN
    RAISE EXCEPTION 'supplier payment replacement source graph is invalid';
  END IF;
  ...
  IF NOT FOUND OR v_event.org_id IS DISTINCT FROM v_replacement.org_id
     ...
     OR v_event.event_type IS DISTINCT FROM 'supplier_payment_replaced'
     OR v_event.source_type IS DISTINCT FROM 'supplier_payment_replacement'
    ...
    RAISE EXCEPTION 'supplier payment replacement event graph is invalid';
  END IF;
  ...
  IF v_journal.id IS NULL OR v_original_journal.id IS NULL
     ...
     OR v_journal.source_module IS DISTINCT FROM 'ap_payment_replacement'
    ...
    RAISE EXCEPTION 'supplier payment replacement journal graph is invalid';
  END IF;
  ...
  IF v_period.id IS NULL OR v_period.org_id IS DISTINCT FROM v_replacement.org_id
    ...
    RAISE EXCEPTION 'supplier payment replacement period graph is invalid';
  END IF;
  ...
  IF v_line_count IS DISTINCT FROM 2
    ...
    RAISE EXCEPTION 'supplier payment replacement journal lines are not an exact copy';
  END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.validate_supplier_payment_replacement_trigger()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
...
BEGIN
  ...
  PERFORM public.validate_supplier_payment_replacement_graph(v_replacement_id);
  RETURN NULL;
END;
$$;

DROP TRIGGER IF EXISTS guard_supplier_payment_replacement_write
  ON public.supplier_payment_replacements;
CREATE TRIGGER guard_supplier_payment_replacement_write
  BEFORE INSERT OR UPDATE OR DELETE ON public.supplier_payment_replacements
  FOR EACH ROW EXECUTE FUNCTION public.guard_supplier_payment_replacement_write();
DROP TRIGGER IF EXISTS guard_supplier_payment_replacement_truncate
  ON public.supplier_payment_replacements;
CREATE TRIGGER guard_supplier_payment_replacement_truncate
  BEFORE TRUNCATE ON public.supplier_payment_replacements
  FOR EACH STATEMENT EXECUTE FUNCTION public.guard_supplier_payment_replacement_write();
DROP TRIGGER IF EXISTS validate_supplier_payment_replacement_deferred
  ON public.supplier_payment_replacements;
CREATE CONSTRAINT TRIGGER validate_supplier_payment_replacement_deferred
  AFTER INSERT OR UPDATE OR DELETE ON public.supplier_payment_replacements
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE FUNCTION public.validate_supplier_payment_replacement_trigger();
...
CREATE OR REPLACE FUNCTION public.post_supplier_payment_replacement(
...
BEGIN
  ...
  IF p_replacement_number IS NULL
    ...
    RAISE EXCEPTION 'invalid supplier payment replacement request';
  END IF;
  ...
  FROM public.supplier_payment_replacements
  ...
  IF v_existing_id IS NOT NULL THEN
    IF v_existing_correction_id IS DISTINCT FROM p_correction_id
      ...
      RAISE EXCEPTION 'supplier payment replacement idempotency key conflicts with another payload';
    END IF;
    PERFORM public.validate_supplier_payment_replacement_graph(v_existing_id);
    RETURN v_existing_id;
  END IF;
  SELECT * INTO v_correction FROM public.supplier_payment_corrections
  WHERE id = p_correction_id AND org_id = v_actor_org FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'corrected supplier payment not found or unavailable';
  END IF;
  PERFORM public.validate_supplier_payment_correction_graph(v_correction.id);
  SELECT * INTO v_payment FROM public.supplier_payments
  WHERE id = v_correction.original_payment_id AND org_id = v_actor_org FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'corrected supplier payment not found or unavailable';
  END IF;
  SELECT * INTO v_original_journal FROM public.journal_entries
  ...
  IF v_original_journal.id IS NULL
     ...
     OR v_correction.original_payment_id IS DISTINCT FROM v_payment.id THEN
    RAISE EXCEPTION 'supplier payment correction is not replaceable';
  END IF;
  IF EXISTS (SELECT 1 FROM public.supplier_payment_replacements
    WHERE original_correction_id = v_correction.id) THEN
    RAISE EXCEPTION 'supplier payment correction already has a replacement';
  END IF;
  IF p_replacement_date < v_correction.correction_date THEN
    RAISE EXCEPTION 'supplier payment replacement date cannot precede the correction date';
  END IF;
  ...
  IF v_period_id IS NULL THEN
    RAISE EXCEPTION 'an OPEN accounting period is required for the supplier payment replacement';
  END IF;
  PERFORM set_config('tapaano.accounting_write', 'trusted', true);
  PERFORM set_config('tapaano.supplier_payment_replacement_write', 'trusted', true);
  INSERT INTO public.accounting_events (
    ...
    v_payment.org_id, v_payment.entity_id, 'supplier_payment_replaced',
    'supplier_payment_replacement', v_replacement_id, p_idempotency_key,
  ...
  INSERT INTO public.journal_entries (
    ...
    'SPR-' || left(p_replacement_number, 36) || '-'
      || left(md5(p_idempotency_key), 8),
    p_replacement_date,
    'Replacement supplier payment ' || p_replacement_number || ': ' || p_reference,
    'posted', v_actor, now(), 'ap_payment_replacement', v_period_id, v_event_id
  ...
  SELECT v_journal_id, account_id, debit, credit,
    'Replacement supplier payment: ' || COALESCE(memo, 'exact copy'),
    org_id, entity_id, line_number
  FROM public.journal_lines
  WHERE journal_entry_id = v_original_journal.id
  ORDER BY line_number;
  INSERT INTO public.supplier_payment_replacements (
    id, org_id, entity_id, vendor_id, bill_id, original_payment_id,
    ...
    v_replacement_id, v_payment.org_id, v_payment.entity_id,
    v_payment.vendor_id, v_payment.bill_id, v_payment.id, v_correction.id,
    p_replacement_number, p_replacement_date, upper(v_payment.currency),
    v_payment.amount, p_reference, v_event_id, v_journal_id,
...
BEGIN
  FOR replacement_record IN SELECT id FROM public.supplier_payment_replacements LOOP
    PERFORM public.validate_supplier_payment_replacement_graph(replacement_record.id);
  END LOOP;
END;
$$;

ALTER TABLE public.supplier_payment_replacements ENABLE ROW LEVEL SECURITY;
...
BEGIN
  ...
  LOOP
    EXECUTE format(
      'DROP POLICY IF EXISTS %I ON public.supplier_payment_replacements',
      policy_record.policyname
    );
  END LOOP;
  CREATE POLICY supplier_payment_replacements_tenant_read
    ON public.supplier_payment_replacements FOR SELECT TO authenticated
    USING (org_id = public.get_user_org_id());
  REVOKE ALL ON TABLE public.supplier_payment_replacements
    FROM PUBLIC, anon, authenticated, service_role;
  FOR column_record IN SELECT column_name FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'supplier_payment_replacements'
  LOOP
    EXECUTE format(
      'REVOKE ALL PRIVILEGES (%I) ON TABLE public.supplier_payment_replacements FROM PUBLIC',
      column_record.column_name
    );
    FOREACH role_name IN ARRAY ARRAY['anon', 'authenticated', 'service_role'] LOOP
      EXECUTE format(
        'REVOKE ALL PRIVILEGES (%I) ON TABLE public.supplier_payment_replacements FROM %I',
        column_record.column_name, role_name
      );
    END LOOP;
  END LOOP;
  GRANT SELECT ON TABLE public.supplier_payment_replacements TO authenticated;
  IF EXISTS (SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND schemaname = 'public'
      AND tablename = 'supplier_payment_replacements') THEN
    ALTER PUBLICATION supabase_realtime
      DROP TABLE public.supplier_payment_replacements;
  END IF;
  FOR routine IN SELECT namespace.nspname, procedure_info.proname,
    ...
    WHERE namespace.nspname = 'public'
      AND procedure_info.proname = 'post_supplier_payment_replacement'
  LOOP
...
REVOKE ALL ON FUNCTION public.validate_supplier_payment_replacement_graph(uuid)
  FROM PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.post_supplier_payment_replacement(
  uuid, text, date, text, text
) FROM PUBLIC, anon, service_role;
GRANT EXECUTE ON FUNCTION public.post_supplier_payment_replacement(
  uuid, text, date, text, text
) TO authenticated;
