...
interface SupplierPaymentReplacementFormProps {
  correctionId: string;
...
export function SupplierPaymentReplacementForm({
  correctionId,
  correctionNumber,
  correctionDate,
  currency,
  amount,
}: SupplierPaymentReplacementFormProps) {
  ...
  const postReplacement = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase.rpc("post_supplier_payment_replacement", {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="gap-2">
          <RefreshCw className="h-3.5 w-3.5" /> Replace payment
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ShieldCheck className="h-5 w-5" /> Replace corrected payment {correctionNumber}
          </DialogTitle>
          <DialogDescription>
            ...
            the browser. This is not evidence of bank execution, recall, match, or reconciliation.
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); postReplacement.mutate(); }}>
          <div className="space-y-2">
            <Label>Replacement payment number</Label>
          ...
          <Button type="submit" className="w-full" disabled={postReplacement.isPending}>
            {postReplacement.isPending ? "Posting…" : "Post one replacement payment"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
