...
function Workspace(){
 ...
 return <div className="space-y-5"><p>Credits reduce the original invoice by line. Any amount already settled becomes a customer balance for future invoices or refunds. Record confirmed bank refunds here. To approve and send a Stripe refund, open Provider refunds in Finance integrations.</p>
...
function CustomerDetail({report:r,canWrite,cash}:{report:Report;canWrite:boolean;cash:{value:string;label:string}[]}){
 ...
 return <section className="space-y-5">
  ...
  {credits.slice(page*20,page*20+20).map(c=>{const i=r.invoices.find(i=>i.id===c.invoiceId);return <article key={c.id} className="rounded border p-4 space-y-3"><h3 className="font-semibold">{c.reference} · {i?.number} · {i?.customerName}</h3><p>{c.date} · Credit {c.amount} · Receivable reduction {c.arAmount} · Customer balance created {c.balanceAmount} · Balance {c.remaining} · Reserved now {c.providerReserved} · Available to use {c.availableToUse} {r.currency}{c.reversedOn?` · Reversed ${c.reversedOn}`:''}</p>
   {canWrite&&!c.reversedOn&&c.availableToUse!=='0.00'?<div className="grid gap-4 lg:grid-cols-2"><FinanceActionForm title="Request confirmed customer refund" fields={[
   ...
   {c.uses.map(u=><div key={u.id} className="rounded border p-3"><p>{u.kind==='REFUND'?'Refund':'Application'} · {u.reference} · {u.date} · {u.amount}{u.reversedOn?` · Corrected ${u.reversedOn}`:''}</p>{canWrite&&!u.providerRefundId&&!u.reversedOn&&!c.reversedOn?<details><summary>Correct this accounting record</summary><FinanceActionForm title="Request customer credit use correction" fields={[{name:'date',label:'Customer credit use correction date',type:'date'},{name:'reason',label:'Correction and bank evidence'}]} submit={(v,key)=>requestFinance(r.entityId,'CUSTOMER_CREDIT_USE_REVERSE',{use_id:u.id,date:v.date},v.reason,key)}/></details>:null}</div>)}
   {c.subscriptionContractId?<a className="underline" href={`/contracts?contract=${c.subscriptionContractId}`}>Manage the subscription service change</a>:null}
   {canWrite&&c.providerReserved==='0.00'&&!c.subscriptionChangeId&&!c.reversedOn&&!c.uses.some(u=>!u.reversedOn)?<details><summary>Reverse this credit</summary><FinanceActionForm title="Request customer credit reversal" fields={[{name:'date',label:'Customer credit reversal date',type:'date'},{name:'reason',label:'Credit reversal evidence'}]} submit={(v,key)=>requestFinance(r.entityId,'CUSTOMER_CREDIT_REVERSE',{credit_id:c.id,date:v.date},v.reason,key)}/></details>:null}
  </article>;})}
