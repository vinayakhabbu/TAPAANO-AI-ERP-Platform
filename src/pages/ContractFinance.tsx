...
import {RevenueRevisions} from '@/components/finance/RevenueRevisions';
...
function ContractEditor(){
 ...
 return <div className="space-y-6"><p>Approve the contract and its performance obligations before billing. Fixed subscriptions recognize service by day; milestones require transfer evidence. Usage is finalized against a source control total and billed in arrears. Revenue earned before invoicing is recorded as an unbilled receivable.</p>
  <p className="text-sm text-muted-foreground">Billing uses the entity's functional currency. Include a sourced tax assessment when tax applies; collected tax is excluded from the revenue schedule. Revenue policy and contract terms require finance review. Forecast schedules do not post revenue.</p>
  {history.isError||entities.isError||accounts.isError?<p role="alert" className="text-destructive">Contract history or account setup unavailable.</p>:canWrite?<details><summary className="cursor-pointer font-semibold">Create a contract</summary><FinanceActionForm title="Request contract approval" fields={[
   ...
   {name:'tax_required',label:'Tax assessment required for every bill',value:'false',options:[{value:'false',label:'No contract-wide requirement'},{value:'true',label:'Require tax or exemption assessment'}]},
   {name:'reason',label:'Contract and accounting policy evidence'},
  ]} submit={(v,key)=>requestFinance(v.entity,'CONTRACT_CREATE',{customer_id:v.customer,reference:v.reference,kind:v.kind,...(v.tax_required==='true'?{tax_required:true}:{}),starts_on:v.starts_on,ends_on:v.ends_on,cycle_months:Number(v.months),price:v.price,unit_price:v.unit_price,timezone:v.timezone,deferred_account_id:v.deferred,unbilled_account_id:v.unbilled,
...
function ContractDetail({report:r,canWrite}:{report:Report;canWrite:boolean}){
 return <section className="space-y-5"><h2 className="text-lg font-semibold">{r.reference} · {r.currency} · as of {r.asOf}</h2><dl className="grid gap-3 sm:grid-cols-4">{[['Net billed',r.billed],['Recognized revenue',r.recognized],['Deferred revenue',r.deferred],['Unbilled receivable',r.unbilled]].map(([label,value])=><div key={label} className="rounded border p-3"><dt>{label}</dt><dd className="font-mono">{value}</dd></div>)}</dl>
  ...
  {!r.cycles.some(c=>c.revisions.length)?<SubscriptionLifecycle contractId={r.id} entityId={r.entityId} canWrite={canWrite}/>:null}
  ...
  {r.cycles.map(c=><details key={c.id} className="rounded-lg border p-4"><summary className="cursor-pointer font-semibold">Cycle {c.number} · {c.startsOn} to {c.endsOn} · {c.price} {r.currency} · {c.cancelled?'Cancelled':c.creditId?'Credited':c.billingRequest?'Billed':'Unbilled'}</summary><div className="mt-4 space-y-4">
   {r.terms.kind==='FIXED'&&c.invoiceId&&!c.cancelled&&!c.creditId?<RevenueRevisions entity={r.entityId} cycle={c} canWrite={canWrite}/>:null}
   ...
   {canWrite&&!c.cancelled&&!c.creditId?<div className="grid gap-4 lg:grid-cols-2">
    ...
    {c.usageFinalized&&!c.billingRequest?<FinanceActionForm title="Request billing" fields={[{name:'number',label:'Invoice number'},{name:'date',label:'Invoice date',type:'date'},{name:'due',label:'Invoice due date',type:'date'},{name:'reason',label:'Billing evidence'}]} submit={(v,key)=>{const tax=readTaxAssessment(v);return requestFinance(r.entityId,'CONTRACT_BILL',{cycle_id:c.id,number:v.number,issue_date:v.date,due_date:v.due,...(tax?{tax}:{})},v.reason,key);}}><TaxAssessmentFields optional/></FinanceActionForm>:null}
    {c.usageFinalized?<FinanceActionForm title="Request revenue recognition" fields={[{name:'date',label:'Recognize service through',type:'date'},{name:'reason',label:'Revenue recognition evidence'}]} submit={(v,key)=>requestFinance(r.entityId,'CONTRACT_RECOGNIZE',{cycle_id:c.id,as_of:v.date,evidence:(c.revisions.at(-1)?.plan??c.allocations).filter(a=>a.method==='MILESTONE'&&v['satisfied-'+a.key]).map(a=>({key:a.key,satisfied_on:v['satisfied-'+a.key],reference:v['evidence-'+a.key]}))},v.reason,key)}>
      {(c.revisions.at(-1)?.plan.map(a=>({...a,description:a.key}))??c.allocations).filter(a=>a.method==='MILESTONE').map(a=><div key={a.key} className="grid gap-2 sm:grid-cols-2"><label>{a.description} transferred on<input type="date" name={'satisfied-'+a.key} className="block w-full rounded border bg-background p-2"/></label><label>{a.description} acceptance evidence<input name={'evidence-'+a.key} className="block w-full rounded border bg-background p-2"/></label></div>)}
    </FinanceActionForm>:null}
    {c.invoiceId&&!c.revisions.length?<FinanceActionForm title="Request full contract credit" fields={[{name:'number',label:'Credit note number'},{name:'date',label:'Credit date',type:'date'},{name:'reason',label:'Credit evidence'}]} submit={(v,key)=>requestFinance(r.entityId,'CONTRACT_CREDIT',{cycle_id:c.id,number:v.number,date:v.date},v.reason,key)}/>:null}
   </div>:null}
  </div></details>)}
