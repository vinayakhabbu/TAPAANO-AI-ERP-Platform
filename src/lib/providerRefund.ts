...
const balance=z.object({id:z.string().regex(/^txn_[A-Za-z0-9]+$/),source:z.string().regex(/^re_[A-Za-z0-9]+$/),amount:signed,fee:signed.optional(),net:signed.optional(),currency:z.literal('USD'),type:z.enum(['refund','refund_failure','payment_refund','payment_failure_refund']),created:timestamp});
...
export function parseProviderRefundReport(value:unknown){
 ...
 for(const j of r.refunds){check(j.entityId===r.entityId&&!seen.has(j.id)&&signedCents(j.amount)>0n);seen.add(j.id);check((j.useId===null)===(j.journalId===null));check((j.reversedOn===null)===(j.reversalJournalId===null));check(!j.canceled||!j.dispatchStartedAt&&!j.providerId&&!j.useId);check((j.proof===null)===(j.providerId===null));
  if(j.proof){const p=j.proof;check(j.dispatchStartedAt&&j.lastCheckedAt&&j.observationCount>0&&p.id===j.providerId&&p.jobId===j.id&&p.amount===j.amount);if(p.balance)check(p.balance.source===p.id&&signedCents(p.balance.amount)===-signedCents(j.amount));if(p.failureBalance)check(p.failureBalance.source===p.id&&signedCents(p.failureBalance.amount)===signedCents(j.amount));for(const b of [p.balance,p.failureBalance])if(b){check((b.fee===undefined)===(b.net===undefined));if(b.fee!==undefined&&b.net!==undefined)check(signedCents(b.net)===signedCents(b.amount)-signedCents(b.fee));}const fee=signedCents(p.balance?.fee??'0.00'),retained=fee+signedCents(p.failureBalance?.fee??'0.00');check(fee>=0n&&fee<=signedCents(j.maximumFee)&&retained>=0n&&retained<=signedCents(j.maximumFee));if(p.status==='succeeded')check(p.balance&&!p.failureBalance);if(['failed','canceled'].includes(p.status)&&p.balance)check(p.failureBalance);}
 }
 return r;
}
