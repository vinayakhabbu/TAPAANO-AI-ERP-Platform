...
export async function qualifyBankFeedWorkflow({rpc,clientA,clientB,clientReviewer,browser,ids,email,password,api,serviceKey,db}){
 ...
 const matchWindow=async statement=>{for(const negative of [false,true]){const view=await rpc(clientA,'get_cash_reconciliation',{p_statement_id:statement});const direction=l=>String(l.amount).startsWith('-')===negative;await rpc(clientA,'match_cash_statement',{p_statement_id:statement,p_bank_lines:view.lines.filter(direction).map(l=>l.id),p_book_lines:view.bookLines.filter(direction).map(l=>l.id),p_reason:'Matched bank references and direction-specific control totals',p_revision:view.revision});}};
 await matchWindow(original);let cash=await rpc(clientA,'get_cash_reconciliation',{p_statement_id:original});
 ...
 await matchWindow(rebuilt);cash=await rpc(clientA,'get_cash_reconciliation',{p_statement_id:rebuilt});assert.equal(cash.variance,'0.00');assert.equal(cash.unmatchedCount,0);
